...
public class SesionesBD {
    ...
    public int insertarsesion(sesiones s) {
        datos = this.conecta.Inserta("INSERT into sesiones (usuario,rol,hora_inicio,hora_cierre)values ('" + s.getUsuario() + "','" + s.getRol() + "','" + s.getHora_inicio() + "','" + s.getHora_cierre() + "')");
        return datos;
    }

    public ResultSet tablaSesion() {

        rs = this.conecta.Consulta("SELECT id_sesion,usuario,(fecha || ' - ' || hora_inicio) AS fya FROM sesiones");
        return rs;
    }
